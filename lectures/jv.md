...
令 $x_t$ 表示在特定公司就职的劳动者在 t 时刻的特定岗位人力资本，$w_t$ 表示当前工资。

令 $w_t = x_t(1 - s_t - \phi_t)$，其中

* $\phi_t$ 是对当前职位的特定岗位人力资本投资，且
* $s_t$ 是用于获取其他公司新工作机会的求职努力程度。

只要劳动者继续留在当前工作，$\{x_t\}$ 的演变由 $x_{t+1} = g(x_t, \phi_t)$ 给出。

当 t 时刻的搜索努力为 $s_t$ 时，劳动者以概率 $\pi(s_t) \in [0, 1]$ 得到新的工作机会。

这个机会的价值（以特定岗位人力资本衡量）是 $u_{t+1}$，其中 $\{u_t\}$ 是具有共同分布 $f$ 的独立同分布序列。
...
函数`operator_factory`接收这个类的实例并返回贝尔曼算子`T`的jit编译版本，即：
